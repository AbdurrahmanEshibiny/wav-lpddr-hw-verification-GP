...
function getCvgViewHyperlinkedName(rowData, dataArrayRowIdx) {
	var result;
	var parentPath = hdlPaths[rowData.parent];
...
function getCvgIndexTableConfigObj(jsonObj) {
	var configObj = {
		...
		columns:
			...
			{
				// name column
				type     : 'num', // sorting is based on 'id'
				...
				render   : {
					type   : 'id', // we need integer comparison between rows 
					sort   : 'id',
					filter : function (cellData, type, fullRowJsonObj, meta) {
						...
						if (fullRowJsonObj.type == g_COVERGROUP_INST) {
							...
							content = 'cvg:' + cellData.name + '#path:' + hdlPaths[cellData.parent] + '/' + cellData.name;
						} else {
...
function getDivElementForComment(title, comment) {
	...
	/* Notice; in the following statement I am setting width:100% so that when
	 * the size of the textarea is smaller than the tooltip, then the textarea
	 * will expand to consume all the available space */
	textArea.setAttribute('style', 'resize:none;width:100%;border:0;');
	
	if (comment.length > 39) {
...
function getTooltipTable(jsonObj)
{
    ...
    // options value items
    if (jsonObj.opts) {
	    for(var i=0 ; i<jsonObj.opts.length ; i++) {
	    	tr = document.createElement('tr');
...
function drawCvgViewTable() {
	var body = document.getElementsByTagName('body')[0];
	var div = document.createElement('div');
	var cvgTable = document.createElement('table');
	var config = getCvgIndexTableConfigObj(cvgViewData);
...
function genDataCvgView(data, scopeIds) {
	for (scopeIdx=1;scopeIdx<scopeIds.length; scopeIdx++) {
		var scopeHash = scopeIds[scopeIdx][0];
		var scopeName = scopeIds[scopeIdx][1];
		var newCvgs = data['g'+ scopeHash];
		hdlPaths[scopeHash] = scopeName;
		for(cvgIdx=0; cvgIdx < newCvgs.length; cvgIdx++) {
			if (newCvgs[cvgIdx].type == g_COVERGROUP_TYPE) {
				newCvgs[cvgIdx].parent = scopeHash;
				newCvgs[cvgIdx].type = g_COVERGROUP_VIEW;
				cvgViewData.push(newCvgs[cvgIdx]);
			}
		}
	}
	cvgViewFileToRead--;
	if (cvgViewFileToRead == 0) {
		drawCvgViewTable();
	}
}

function buildCvgViewTable(dataArray) {
    var scopeIds;
	cvgViewFileToRead = dataArray.length - 1;
	g_LoadDataFp = function(data) {
		return genDataCvgView(data, scopeIds);
	};
	for (i=1; i < dataArray.length; i++) {
		scopeIds = dataArray[i];
		loadCvgDataJsonFile(i, g_CVG_DATA_FILE_NAME_PREFIX);
	}
}
...
function getCvgTypeInstTableConfigObj(jsonObj) {
	var configObj = {
		paging : false,
		info   : false,
		order  : [[0, 'asc']],
			...
			[
			{
				...
				render   : {
					...
					filter: function (cellData, type, rowData, meta) {
						var content;
						if (rowData.type == g_CRX) {
							content = 'Cross ';
						} else if (rowData.type == g_CVP) {
							content = 'Coverpoint ';
						} else { // g_CVP_PA
							content = 'Power State ';
						}
					...
					display: function (cellData, type, rowData, meta) {
						...
						content += getCvpCrxHyperlinkedName(rowData, meta.row);
						return content;
					}
				}
			},
	...
	if (jsonObj.cvpcrx.length > g_TABLE_REASONABLE_LENGTH) {
		configObj.paging = true;
		configObj.info   = true;
		configObj.deferRender = true;
		configObj.lengthMenu = g_TABLE_LENGTH_MENUE;
...
function drawCvgTypeInstTable(jsonObj) {
	var table, tbody;
	...
	$(table).DataTable(getCvgTypeInstTableConfigObj(jsonObj));
}
...
function drawTypeInstPage(dataArray, typeInstScopeId) {
	...
	drawCvgTypeInstTable(typeInstObj);
}
...
function drawCvpCrxPageHeader(jsonObj) {
	...
	h2.innerHTML = cvpOrCrxString;
	div.appendChild(h2);
	
	h2 = document.createElement('h2');
	h2.innerHTML = jsonObj[g_oCONST.gBinObjIdx.name];
	div.appendChild(h2);
}
...
//  Drop the support of truncating long bin name
//	var content;
//	content = getBinTypeStr(rowData[g_oCONST.gBinArrIdx.type]) + ' ';
//	if ((rowData[g_oCONST.gBinArrIdx.name]).length > g_MAX_NAME_LENGTH) {
//		content += getTruncatedHyperlinkedName(
//				rowData[g_oCONST.gBinArrIdx.name], /*name*/
//				0,               /*id*/
//				0,               /*fileNum*/
//				dataArrayRowIdx, /*dataArrayRowIdx*/
//				0,               /*isHyperlink*/
//				0                /*fnGetHyperlink*/);
//	} else {
//		content += rowData[g_oCONST.gBinArrIdx.name];
//	}
//	return content;
...
function getCvpCrxBinsTableConfigObj(jsonObj) {
	var configObj = {
		paging : false,
		info   : false,
		order  : [[0, 'desc' ]],
		data: jsonObj[g_oCONST.gBinObjIdx.binArr],
		createdRow: function (rowDomObj, rowData, rowDataIdx) {
...
//					Drop the support of truncating bin name if it is too long.
//					this is better for performance and visibility
//
//					/* check long name tooltip */
//					if (cellData[g_oCONST.gBinArrIdx.name].length > g_MAX_NAME_LENGTH) {
//						$(cellDomObj).find('.truncate').each(function () {
//							enableBinLongNameTooltip($(this));
//						});
//					}
				},
				render: {
					type   : g_oCONST.gBinArrIdx.h, // return any numerical value
					sort   : g_oCONST.gBinArrIdx.type,
					filter : function (cellData, type, rowData, meta) {
						var content;
						content = getBinTypeStr(cellData[g_oCONST.gBinArrIdx.type])
							+ ' '
							+ rowData[g_oCONST.gBinArrIdx.name];
						
						if (g_oCONST.showExcluded) {
							if (cellData[g_oCONST.gBinArrIdx.isExcluded]) {
								content += ' Excluded ' + cellData[g_oCONST.gBinArrIdx.h];
								if (cellData[g_oCONST.gBinArrIdx.isExcluded][g_BIN_EXCLUSION_COMMENT_INDEX]) {
									content += ' Exclusion Comment';
								}
							}
						}
	...
	if ((jsonObj[g_oCONST.gBinObjIdx.binArr]).length > g_TABLE_REASONABLE_LENGTH) {
		configObj.paging = true;
		configObj.info   = true;
		configObj.deferRender = true;
		configObj.lengthMenu = g_TABLE_LENGTH_MENUE;
...
function drawCvpCrxBinsTable(jsonObj) {
	var table, tbody;
